...
PCD_DIR="$PKG/PCD"
OUT="$PCD_DIR/scans_mid360_test.pcd"
RUN_DIR="$WS/results/mid360_test_$(date +%Y%m%d_%H%M%S)"
...
echo "CONFIG=mid360" | tee -a "$RUN_DIR/status.log"
echo "BAG=$BAG" | tee -a "$RUN_DIR/status.log"
echo "RATE=1.0" | tee -a "$RUN_DIR/status.log"
echo "START $(date '+%F %T')" | tee -a "$RUN_DIR/status.log"

roslaunch fast_lio mapping_mid360.launch rviz:=false > "$RUN_DIR/roslaunch.log" 2>&1 &
LAUNCH_PID=$!
