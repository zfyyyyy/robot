#!/usr/bin/env bash
set -eo pipefail
WS="$HOME/Desktop/fastlio2_ws"
PKG="$WS/src/FAST_LIO"
BAG="$WS/bags/r3live/hku_main_building.bag"
PCD_DIR="$PKG/PCD"
OUT="$PCD_DIR/scans_school.pcd"
RUN_DIR="$WS/results/school_run_$(date +%Y%m%d_%H%M%S)"
LAUNCH="$PKG/launch/mapping_avia_school_save.launch"
mkdir -p "$RUN_DIR" "$PCD_DIR"
cd "$WS"
source /opt/ros/noetic/setup.bash
source devel/setup.bash

cat > "$LAUNCH" <<'XML'
<launch>
  <arg name="rviz" default="false" />
  <rosparam command="load" file="$(find fast_lio)/config/avia.yaml" />

  <!-- Lower-density settings for long bags in VMware, to reduce memory pressure. -->
  <param name="feature_extract_enable" type="bool" value="0"/>
  <param name="point_filter_num" type="int" value="6"/>
  <param name="max_iteration" type="int" value="3" />
  <param name="filter_size_surf" type="double" value="1.0" />
  <param name="filter_size_map" type="double" value="1.0" />
  <param name="cube_side_length" type="double" value="1000" />
  <param name="runtime_pos_log_enable" type="bool" value="0" />

  <param name="publish/path_en" type="bool" value="true" />
  <param name="publish/scan_publish_en" type="bool" value="false" />
  <param name="publish/dense_publish_en" type="bool" value="false" />
  <param name="publish/scan_bodyframe_pub_en" type="bool" value="false" />

  <param name="pcd_save/pcd_save_en" type="bool" value="true" />
  <param name="pcd_save/interval" type="int" value="-1" />

  <node pkg="fast_lio" type="fastlio_mapping" name="laserMapping" output="screen" />
  <group if="$(arg rviz)">
    <node launch-prefix="nice" pkg="rviz" type="rviz" name="rviz" args="-d $(find fast_lio)/rviz_cfg/fastlio2_live.rviz" />
  </group>
</launch>
XML

pkill -f "rosbag play" || true
pkill -f fastlio_mapping || true
pkill -f mapping_avia || true
pkill -f pcd_to_pointcloud || true
pkill -f rviz || true
sleep 2

if [ -f "$OUT" ]; then
  cp "$OUT" "$RUN_DIR/scans_school_before.pcd"
fi
rm -f "$PCD_DIR/scans.pcd"

echo "RUN_DIR=$RUN_DIR" | tee "$RUN_DIR/status.log"
echo "START $(date '+%F %T')" | tee -a "$RUN_DIR/status.log"
echo "BAG=$BAG" | tee -a "$RUN_DIR/status.log"
echo "RATE=1.0" | tee -a "$RUN_DIR/status.log"

roslaunch fast_lio mapping_avia_school_save.launch rviz:=false > "$RUN_DIR/roslaunch.log" 2>&1 &
LAUNCH_PID=$!
echo "$LAUNCH_PID" > "$RUN_DIR/roslaunch.pid"
sleep 8

rosbag play "$BAG" -r 1.0 > "$RUN_DIR/rosbag.log" 2>&1
BAG_STATUS=$?
echo "ROSBAG_DONE status=$BAG_STATUS $(date '+%F %T')" | tee -a "$RUN_DIR/status.log"

kill -INT "$LAUNCH_PID" 2>/dev/null || true
sleep 20
if kill -0 "$LAUNCH_PID" 2>/dev/null; then
  kill -TERM "$LAUNCH_PID" 2>/dev/null || true
  sleep 5
fi
wait "$LAUNCH_PID" 2>/dev/null || true

if [ -f "$PCD_DIR/scans.pcd" ]; then
  mv -f "$PCD_DIR/scans.pcd" "$OUT"
  ls -lh "$OUT" | tee -a "$RUN_DIR/status.log"
  echo "DONE $(date '+%F %T')" | tee -a "$RUN_DIR/status.log"
else
  echo "ERROR: scans.pcd not generated" | tee -a "$RUN_DIR/status.log"
  exit 2
fi